...
export async function GET(request: Request) {
  try {
    ...
    if (!districtId) {
      return NextResponse.json(
        { success: false, error: "Parameter 'districtId' wajib diisi." },
        { status: 400 }
      );
    }

    // API Call (dengan Header Key RajaOngkir): GET https://rajaongkir.komerce.id/api/v1/destination/sub-district/[districtId]
    ...
    if (!res.ok || (data.meta && data.meta.code !== 200)) {
      return NextResponse.json(
        { success: false, error: data.meta?.message || "Gagal mengambil data kelurahan." },
        { status: 400 }
      );
    }
