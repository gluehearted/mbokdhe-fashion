...
import CustomerForm, { Customer, COURIER_OPTIONS } from "./CustomerForm";
import ProductForm, { Product } from "./ProductForm";

const AVAILABLE_COURIERS = COURIER_OPTIONS.map((c) => ({ code: c, label: c }));
...
export default function OrderForm({ orderId }: OrderFormProps) {
  ...
  // --- State Modal Tambah Pelanggan Baru & Produk Baru ---
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        let availableProds: Product[] = [];
        if (prodData.success) {
          availableProds = prodData.data.filter((p: Product) => p.status === "Tersedia");
        }
        ...
        if (orderId) {
          ...
          if (orderJson.success && orderJson.data) {
            ...
            let loadedStatus = order.status || "Menunggu";
            if (loadedStatus === "Keep") {
              if ((order.dpAmount ?? 0) >= (order.totalPrice ?? 0) && (order.totalPrice ?? 0) > 0) {
                loadedStatus = "Keep (Lunas)";
              } else {
                loadedStatus = "Keep (Belum Bayar)";
              }
            }
            setOrderStatus(loadedStatus);

            setManualShippingCost(String(order.shippingCost ?? 0));
            setDpAmountInput(String(order.dpAmount ?? 0));
            setOrderNotes(order.notes || "");
            setSelectedCourier(order.shippingCourier || "");

            const orderProds: Product[] = order.products || [];
            const mergedProdsMap = new Map<string, Product>();

            availableProds.forEach((p) => mergedProdsMap.set(p.id, p));
            ...
            setAvailableProducts(Array.from(mergedProdsMap.values()));

            const prodIds = orderProds.map((p) => p.id);
            setSelectedProductIds(prodIds);

            const customPricesObj: Record<string, string> = {};
            const discountsObj: Record<string, string> = {};

            orderProds.forEach((p) => {
              customPricesObj[p.id] = String(p.customPrice !== undefined && p.customPrice !== null ? p.customPrice : p.price);
              discountsObj[p.id] = String(p.discount || 0);
            });

            setCustomPrices(customPricesObj);
            setIndividualDiscounts(discountsObj);
          } else {
            setErrorMessage(orderJson.error || "Pesanan tidak ditemukan.");
          }
        } else {
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let finalDpAmount = 0;
      if (orderStatus === "DP") {
        finalDpAmount = parseInt(dpAmountInput, 10) || 0;
      } else if (
        orderStatus === "Keep (Lunas)" ||
        orderStatus === "Siap Kirim" ||
        orderStatus === "Dikirim"
      ) {
        finalDpAmount = totalTagihan;
      } else {
        finalDpAmount = 0;
      }
      ...
      const payload = {
        ...
        shippingCourier: selectedCourier || selectedCustomer?.courier || "JNE",
        status: orderStatus,
      ...
      if (!res.ok || !result.success) {
        throw new Error(result.error || `Gagal ${isEditMode ? "memperbarui" : "membuat"} pesanan.`);
      }
  ...
  return (
    <div className="flex-1 flex flex-col h-screen w-full overflow-hidden bg-[#fbfbfa] dark:bg-[#0c0d0f] text-[#111111] dark:text-[#f3f3f3] font-ui transition-colors duration-200">
      ...
      {/* Modal Tambah Pelanggan Baru (menggunakan CustomerForm) */}
      {isNewCustModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-[2px] flex items-center justify-center p-3 sm:p-4" onClick={() => setIsNewCustModalOpen(false)}>
          <div
            className="bg-white dark:bg-[#141517] border border-[#eaeaea] dark:border-slate-800/80 rounded-[8px] w-full max-w-md p-4 sm:p-6 space-y-4 shadow-[0_12px_40px_rgba(0,0,0,0.08)] max-h-[90vh] overflow-y-auto animate-fade-in-up font-ui"
            onClick={(e) => e.stopPropagation()}
          >
            <CustomerForm
              isModal={true}
              onCancel={() => setIsNewCustModalOpen(false)}
              onSubmitSuccess={(createdCustomer) => {
                const updatedCustomers = [createdCustomer, ...customers];
                setCustomers(updatedCustomers);
                handleCustomerChange(createdCustomer.id, updatedCustomers);
      ...
      {/* Modal Tambah Produk Baru (menggunakan ProductForm) */}
      {isNewProdModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-[2px] flex items-center justify-center p-3 sm:p-4" onClick={() => setIsNewProdModalOpen(false)}>
          <div
            className="bg-white dark:bg-[#141517] border border-[#eaeaea] dark:border-slate-800/80 rounded-[8px] max-w-md w-full p-4 sm:p-5 space-y-4 shadow-[0_12px_40px_rgba(0,0,0,0.04)] max-h-[90vh] overflow-y-auto transition-colors font-ui animate-fade-in-up"
            onClick={(e) => e.stopPropagation()}
          >
            <ProductForm
              isModal={true}
              shops={shops}
              onCancel={() => setIsNewProdModalOpen(false)}
              onSaveAndAddAnother={async (createdProduct) => {
              ...
              onSubmitSuccess={async (createdProduct) => {
                await refreshProductsAndShops();
                if (createdProduct && createdProduct.id) {
                  setSelectedProductIds((prev) => [...prev, createdProduct.id]);
                  setCustomPrices((prev) => ({
      ...
      <header className="flex justify-between items-center w-full pl-14 pr-4 md:px-6 h-16 bg-white dark:bg-[#141517] border-b border-[#eaeaea] dark:border-slate-800/80 z-30 sticky top-0 shrink-0 transition-colors">
        <div className="flex flex-col">
          <h1 className="text-sm font-bold text-[#111111] dark:text-[#f3f3f3] tracking-tight uppercase font-technical">
            {isEditMode ? `Edit Pesanan #${orderId?.slice(0, 8).toUpperCase()}` : "Buat Pesanan (Checkout)"}
          </h1>
          <span className="text-[10px] text-[#787774] dark:text-slate-400 font-technical uppercase mt-0.5 tracking-wider">
            {isEditMode ? "[ Edit Terminal ]" : "[ Checkout Terminal ]"}
          </span>
        </div>
        <Link
          href="/orders"
          ...
          <form onSubmit={handleSubmitOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-6 max-w-6xl mx-auto font-ui">
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-white dark:bg-[#141517] border border-[#eaeaea] dark:border-slate-800/80 rounded-[8px] p-5 space-y-4 shadow-[0_2px_8px_rgba(0,0,0,0.01)] text-xs transition-colors">
                ...
                <div className="space-y-1.5">
                  <label className="block text-[10px] font-bold text-[#787774] dark:text-slate-400 uppercase tracking-widest font-technical">Status Pesanan</label>
                  <select
                    value={orderStatus}
                    onChange={(e) => {
                  ...
                  >
                    ...
                    <option value="Keep (Lunas)">Keep (Lunas - Tahan Kirim)</option>
              ...
              <div className="bg-white dark:bg-[#141517] border border-[#eaeaea] dark:border-slate-800/80 rounded-[8px] p-5 space-y-4 shadow-[0_2px_8px_rgba(0,0,0,0.01)] text-xs transition-colors">
                ...
                <div className="space-y-3 font-technical uppercase">
                  {totalDiscountAmount > 0 ? (
                    <>
                      ...
                      <div className="flex justify-between items-center text-[#787774] dark:text-slate-400 pt-1 border-t border-dashed border-[#eaeaea] dark:border-slate-800">
                ...
                >
                  {submitting ? "Memproses..." : (isEditMode ? "Simpan Perubahan Pesanan" : "Buat Pesanan")}
                </button>
              </div>
